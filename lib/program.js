...
const debug = require('debug')('afd:cli:program')

/*
 * we use this hack to notify child processes that they were invoked
 * from the `dcdn(1)` shell command
 */
const kFromDCDNParentFlag = 'from-afd-parent'
if (process.argv.join('').indexOf(kFromDCDNParentFlag) > -1) {
  removeCFSParentFlagFromArgv(process.argv)
  process.title = `[afd/${basename(process.argv[1]).replace('afd-', '')}]`
} else {
...
function removeCFSParentFlagFromArgv(args) {
  for (let i = 0; i < args.length; ++i) {
    const arg = args[i]
    if (arg.replace('--', '') === kFromDCDNParentFlag) {
      args.splice(i, 1)
    }
  }
}
